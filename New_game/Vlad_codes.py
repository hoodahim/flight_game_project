...
You have had this bucket list for a while now and decided it's time to put things finally into motion!
- You wish to visit 5 locations in Europe and you hope that each has a different weather condition compared to the ones you have already visited. 
...
while total_score < 2500 and CO2_emission < 4500:
    # the ICAO_code_input answers are saved to this list so they cannot be replicated
    print(answer_list)
    ...
    ICAO_code_input = input(
        "Please, write down the preferred ICAO code: ").upper()
    ...
    if ICAO_code_input in answer_list:
        ...
        ICAO_code_input = input("Please, write down the preferred ICAO code: ").upper()
    else:
        ...
        if ICAO_code_input in airports:
            ...
            print(
                f"Your current score is {total_score}, and CO2 emission is {CO2_emission}")
