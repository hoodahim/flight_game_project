...
class Airport:
    ...
    def find_nearby_airports(self):
        ...
        sql = "SELECT airport.ident, airport.name, airport.latitude_deg, airport.longitude_deg, airport.municipality, country.name FROM airport, country WHERE airport.iso_country=country.iso_country AND airport.type= '" + "large_airport" + "'" "AND country.continent = '" + "EU" + "'"
        print(sql)
        cur = config.conn.cursor()
        cur.execute(sql)
        res = cur.fetchall()
        for r in res:
            if r[0] != self.ident:
                # lisätty data, jottei jokaista kenttää tartte hakea                # uudestaan konstruktorissa
                data = {'name': r[1], 'latitude': r[2], 'longitude': r[3], 'municipality': r[4], "country_name": r[5]}
                print(data)
