...
class Game:
    ...
    def __init__(self, id, loc, consumption, player=None):
        ...
        if id==0:
            ...
            sql += ", " + str(self.status["co2"]["budget"]) + ", '" + loc + "', '" + self.status["name"] + "', '" + str(self.status["scoreboard"]) + "')"
            print(sql)
            ...
            cur2.execute(sql2)
            # find game from DB
            sql = "SELECT id, co2_consumed, co2_budget, location, screen_name,scoreboard FROM Game WHERE id='" + id + "'"
            print(sql)
            cur = config.conn.cursor()
            cur.execute(sql)
            res = cur.fetchall()
